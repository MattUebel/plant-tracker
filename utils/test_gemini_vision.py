...
if not os.getenv("GEMINI_API_KEY"):
    ...
    print("Please set your GEMINI_API_KEY in your .env file or export it in your environment.")
    sys.exit(1)
...
class GeminiVisionTester(GeminiVisionTester):
    ...
    async def extract_structured_data_with_json_mode(self, image_path: str) -> Dict[str, Any]:
        """
        ...
        for i, image_path in enumerate(self.image_paths):
            ...
            try:
                ...
                if "error" in structured_data and self.args.use_json_mode:
                    ...
                    structured_data = await vision_tester.extract_structured_data_with_json_mode(str(image_path))
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Advanced testing utility for Gemini Vision API')
    
    parser.add_argument('--model', default=os.getenv("GEMINI_MODEL", "gemini-2.5-pro-exp-03-25"),
                        help='Gemini model name to use (default: gemini-2.5-pro-exp-03-25 or GEMINI_MODEL from .env)')
    
    parser.add_argument('--image', nargs='+', 
                        help='Path(s) to specific image(s) to process (if not specified, all images in uploads directory will be processed)')
    
    parser.add_argument('--uploads-dir', default=str(Path(__file__).parent.parent / "uploads"),
    ...
    parser.add_argument('--use-json-mode', action='store_true',
